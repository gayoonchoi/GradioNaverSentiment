...
*   **카테고리별 분석**: `계절과 자연 > 자연경관 > 꽃 축제`처럼 세분화된 카테고리를 선택하여, 관련 축제 그룹 전체의 감성을 종합적으로 분석합니다.
...
'단일 키워드 분석' 탭은 사용자가 입력한 특정 키워드(예: '강릉커피축제')에 대한 네이버 블로그 후기들을 심층 분석하여 종합적인 인사이트를 제공합니다.  
  
사용자가 키워드를 입력하고 '분석 시작' 버튼을 누르면, 분석이 완료된 후 **[1. 종합 분석 결과]**와 **[2. 개별 블로그 분석 결과]** 두 개의 메인 탭이 활성화됩니다.  
...
입력한 키워드에 대한 전체 후기 데이터를 종합하여 다음과 같은 분석 정보를 시각화합니다.  
  
* **주요 불만 사항**: 후기들에서 공통적으로 언급되는 불만 및 개선점을 요약합니다.  
* **긍정/부정 비율**: 전체 문장의 감성을 긍정/부정으로 분류하여 비율을 **도넛 차트**로 제공합니다.  
* **검색량 트렌드**: 최근 해당 축제(키워드)의 검색량 추이를 **그래프**로 보여줍니다.  
* **감성 분석 요약**: 종합 감성 점수와 긍정/부정 감성 문장의 출현 빈도를 수치로 제공합니다.  
  
<br>  
  
> #### ☀️🍂 각 계절별 분석 (하위 탭)  
>  
> '종합 분석 결과' 내부에 위치한 '각 계절별 분석' 탭에서는 축제가 개최된 시점을 기준으로 계절별 비교 분석 데이터를 제공합니다.  
>  
> 1.  **계절별 긍정/부정 비율**: 각 계절의 긍정/부정 문장 비율을 **막대 그래프**로 비교합니다.  
> 2.  **계절별 워드 클라우드**: 계절별로 긍정 문장과 부정 문장에서 자주 언급된 키워드를 **워드 클라우드** 형태로 시각화합니다.  
  
---  
  
### 2. 📑 개별 블로그 분석 결과  
  
수집된 개별 블로그 후기 글에 대한 상세 분석 결과를 **표(테이블)** 형태로 제공합니다.  
  
1.  **블로그 분석 목록 (표)**  
    * 표에는 각 블로그 후기별 **감성 점수**, **감성 문장 빈도** 등의 요약 값이 표시됩니다.  
    * 해당 후기글에서 추출된 주요 **긍정/부정 판별 문장**이 함께 제공됩니다.  
  
2.  **개별 후기 상세 팝업 (클릭 시)**  
    * 표 안의 특정 블로그 텍스트(제목 또는 내용)를 클릭하면, 해당 후기글 하나만을 상세 분석하는 팝업 또는 상세 뷰가 나타납니다.  
    * **긍정/부정 판별 문장**: 해당 블로그의 전체 문장을 긍정과 부정으로 분류하여 보여줍니다.  
    * **개별 긍정/부정 비율**: 해당 블로그만의 긍정/부정 비율을 **도넛 차트**로 표시합니다.  
    * **문장별 감성 점수**: 각 문장별 긍정(+)/부정(-) 점수를 시각적으로 보여주는 **막대 그래프**가 출력됩니다.  
