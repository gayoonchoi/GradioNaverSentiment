...
def create_ui():
    ...
    def create_detailed_analysis_outputs(is_category_tab: bool):
        """모든 탭에서 사용할 상세 결과 UI 컴포넌트 그룹을 생성합니다."""
        with gr.Blocks():
            status_output = gr.Textbox(label="분석 상태", interactive=False)
            url_output = gr.Markdown(label="수집된 전체 URL 리스트", visible=not is_category_tab)
            
            with gr.Accordion("종합 분석 결과", open=True):
                with gr.Row():
                    negative_summary_output = gr.Markdown(label="주요 불만 사항 요약", visible=False)
                with gr.Row():
                    negative_download_output = gr.File(label="부정적 의견 요약(CSV) 다운로드", visible=False, scale=1)
        ...
        with gr.Tabs():
            with gr.TabItem("단일 키워드 분석"):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        keyword_outputs = create_detailed_analysis_outputs(is_category_tab=False)
            ...
            with gr.TabItem("키워드 비교 분석"):
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        keyword_outputs_a = create_detailed_analysis_outputs(is_category_tab=False)
                    with gr.Column():
                        gr.Markdown("### 키워드 B 분석 결과")
                        keyword_outputs_b = create_detailed_analysis_outputs(is_category_tab=False)
            ...
            with gr.TabItem("카테고리별 축제 분석"):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        category_outputs = create_detailed_analysis_outputs(is_category_tab=True)
            ...
            with gr.TabItem("카테고리별 축제 비교 분석"):
                ...
                with gr.Row():
                    with gr.Column():
                        compare_outputs_a = create_detailed_analysis_outputs(is_category_tab=True)
                    with gr.Column():
                        compare_outputs_b = create_detailed_analysis_outputs(is_category_tab=True)
